...
`npm run build:development` is safe to run locally without any env vars. `npm run build` is used by Cloudflare Pages and requires `SENTRY_ORG`, `SENTRY_PROJECT`, and `SENTRY_AUTH_TOKEN`.
...
2. **Guess comparison** (`src/utils/compare.ts`): Each guess produces feedback on 5 attributes (ecosystem, race, niveau max, couleur, pv max) with statuses: correct/partial/wrong and directional arrows for numeric fields. Race comparison uses ecosystem grouping for partial matches. Only max values are displayed and compared for niveau and pv. Numeric thresholds use percentage-based ranges with absolute floors: green (correct) = max(10% of target, floor), orange (partial) = max(20% of target, floor). Floors: niveau green 5 / orange 10, pv green 25 / orange 50.
